...
//
// See Geo directory for dynamic allocation given ndiv
...
// Go to Render Scene for graphics bit
    ...
    NodeV2=(D3Dvec*)calloc(Ntotal, sizeof(D3Dvec));

    //INITIALISE USING CONSTRUCTOR OR ELSE!
 ...
 // position vectors for basic octahedron 
    ...
    NodeV2[0].SetVec(x,y,z);
    x=1.0; y=0.0; z=0.0;
    NodeV[1].SetVec(x,y,z);
    NodeV2[1].SetVec(x,y,z);
    x=0.0; y=1.0; z=0.0;
    NodeV[2].SetVec(x,y,z);
    NodeV2[2].SetVec(x,y,z);
    x=-1.0; y=0.0; z=0.0;
    NodeV[3].SetVec(x,y,z);
    NodeV2[3].SetVec(x,y,z);
    x=0.0; y=0.0; z=1.0;
    NodeV[4].SetVec(x,y,z);
    NodeV2[4].SetVec(x,y,z);
    x=0.0; y=0.0; z=-1.0;
    NodeV[5].SetVec(x,y,z);
    NodeV2[5].SetVec(x,y,z);
      ...
      //
	    ...
	    for(int itri=istart;itri<istop;itri++){
		    ...
		    if(oldnode1 < 0){
		       ...
		       NodeV2[n_nodes].SetX( 
				NodeV2[ Triangles[itri].Get2() ].GetX()
			       +(
				NodeV2[ Triangles[itri].Get3() ].GetX()
				-NodeV2[ Triangles[itri].Get2() ].GetX()
				)/2.0
				);
		       NodeV2[n_nodes].SetY( 
				NodeV2[ Triangles[itri].Get2() ].GetY()
			       +(
				NodeV2[ Triangles[itri].Get3() ].GetY()
				-NodeV2[ Triangles[itri].Get2() ].GetY()
				)/2.0
				);
		       NodeV2[n_nodes].SetZ( 
				NodeV2[ Triangles[itri].Get2() ].GetZ()
			       +(
				NodeV2[ Triangles[itri].Get3() ].GetZ()
				-NodeV2[ Triangles[itri].Get2() ].GetZ()
				)/2.0
				);
		    }
		    ...
		    if(oldnode2 < 0){
		       ...
		       NodeV2[n_nodes].SetX( 
				NodeV2[ Triangles[itri].Get1() ].GetX()
			       +(
				NodeV2[ Triangles[itri].Get3() ].GetX()
				-NodeV2[ Triangles[itri].Get1() ].GetX()
				)/2.0
		       ...
		       NodeV2[n_nodes].SetY( 
				NodeV2[ Triangles[itri].Get1() ].GetY()
			       +(
				NodeV2[ Triangles[itri].Get3() ].GetY()
				-NodeV2[ Triangles[itri].Get1() ].GetY()
				)/2.0
		       ...
		       NodeV2[n_nodes].SetZ( 
				NodeV2[ Triangles[itri].Get1() ].GetZ()
			       +(
				NodeV2[ Triangles[itri].Get3() ].GetZ()
				-NodeV2[ Triangles[itri].Get1() ].GetZ()
				)/2.0
...
void RenderScene(CCam & Camera1)
{
    ...
    // 0.0 to 128, small values give sharper specular reflection
      ...
      float line_amb2[]={0.0,0.0,1.0,1.0};  //line's ref of ambient light
      float line_diff[]={0.0,0.0,1.0,0.0};  //line's ref of diffuse light
	...
	/*      cout << "i=" << i << " " << xvals[0] << " " << yvals[0]
		      << " " << zvals[0] << endl;
	      cout << "i=" << i << " " << xvals[1] << " " << yvals[1]
		      << " " << zvals[1] << endl;
	      cout << "i=" << i << " " << xvals[2] << " " << yvals[2]
		      << " " << zvals[2] << endl; */
    ...
    /*             j=0;
                 xvals[j]=NodeV2[ Triangles[i].Get1()].GetX();
                 yvals[j]=NodeV2[ Triangles[i].Get1()].GetY();
                 zvals[j]=NodeV2[ Triangles[i].Get1()].GetZ();

                 j=1;
                 xvals[j]=NodeV2[ Triangles[i].Get2()].GetX();
                 yvals[j]=NodeV2[ Triangles[i].Get2()].GetY();
                 zvals[j]=NodeV2[ Triangles[i].Get2()].GetZ();

                 j=2;
                 xvals[j]=NodeV2[ Triangles[i].Get3()].GetX();
                 yvals[j]=NodeV2[ Triangles[i].Get3()].GetY();
                 zvals[j]=NodeV2[ Triangles[i].Get3()].GetZ();
      ...
      glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, line_spec);
      glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, line_amb2);
      glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, line_spec);
      glMaterialfv(GL_FRONT_AND_BACK, GL_SHININESS, line_shine);
